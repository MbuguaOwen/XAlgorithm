...
async def test_trailing_tp_exit(tmp_path, monkeypatch):
    ...
    q.put_nowait((datetime.utcnow(), 103.0, 0.8, 0.9, 3.0, 0.1))
    q.put_nowait((datetime.utcnow(), 101.8, 0.8, 0.9, 3.0, 0.1))
    await asyncio.sleep(0.2)
    task.cancel()
...
async def test_sl_ratchet_exit(tmp_path, monkeypatch):
    ...
    task = asyncio.create_task(manager.start())
    q.put_nowait((datetime.utcnow(), 100.0, 0.8, 0.9, 3.0, 0.1))
    q.put_nowait((datetime.utcnow(), 102.2, 0.8, 0.9, 3.0, 0.1))
    q.put_nowait((datetime.utcnow(), 103.0, 0.8, 0.9, 3.0, 0.1))
